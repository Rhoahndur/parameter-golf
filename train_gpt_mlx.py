...
class Hyperparameters:
    ...
    val_max_tokens: int = int(os.environ.get("VAL_MAX_TOKENS", 0))  # 0 = full val set; set smaller for fast local smoke tests
    ...
    vocab_size: int = int(os.environ.get("VOCAB_SIZE", 1025))  # 1024 tokenizer + 1 MASK token
    mask_token_id: int = 1024  # never in data; injected by masking during MDLM training
    mask_ratio_eps: float = float(os.environ.get("MASK_RATIO_EPS", 0.01))  # floor for t
    curriculum: bool = bool(int(os.environ.get("CURRICULUM", "0")))  # 1 = Beta curriculum for mask ratio
...
class TokenLoader:
    ...
    def next_batch(self, batch_tokens: int, seq_len: int) -> mx.array:
        # MDLM: return complete sequences, no AR shift. Masking happens in training loop.
        usable = (batch_tokens // seq_len) * seq_len
        if usable <= 0:
            raise ValueError(f"token budget too small for seq_len={seq_len}")
        chunk = self.stream.take(usable)
        tokens = chunk.reshape(-1, seq_len)
...
class CausalSelfAttention(nn.Module):
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        y = mx.fast.scaled_dot_product_attention(q, k, v, scale=self.scale, mask=None)  # MDLM: bidirectional
...
class GPT(nn.Module):
    ...
    def loss(
        self,
        masked_ids: mx.array,
        original_ids: mx.array,
        mask: mx.array,
        mask_ratio: mx.array,
    ) -> mx.array:
        # MDLM loss: predict original tokens at masked positions, weighted by 1/t.
        # mask: [B, S] float 0/1. mask_ratio: [B] the per-sequence mask fraction t.
        x = self(masked_ids).reshape(-1, self.tok_emb.weight.shape[1])
        targets = original_ids.reshape(-1)
        logits_proj = x @ self.tok_emb.weight.astype(x.dtype).T
        logits = self.softcap(logits_proj)
        # Per-token CE (no reduction)
        ce = nn.losses.cross_entropy(logits.astype(mx.float32), targets, reduction="none")  # [B*S]
        # Weight = mask * (1/t): zero at unmasked, 1/t at masked
        B = masked_ids.shape[0]
        S = masked_ids.shape[1]
        flat_mask = mask.reshape(-1)  # [B*S]
        inv_t = (1.0 / mask_ratio)[:, None]  # [B, 1]
        inv_t_expanded = mx.broadcast_to(inv_t, (B, S)).reshape(-1)  # [B*S]
        w = flat_mask * inv_t_expanded
        return mx.sum(ce * w) / mx.sum(w)
...
def load_validation_tokens(pattern: str, seq_len: int) -> np.ndarray:
    ...
    usable = (tokens.size // seq_len) * seq_len  # MDLM: no +1 shift needed
    if usable <= 0:
        raise ValueError(f"Validation split is too short for TRAIN_SEQ_LEN={seq_len}")
    return tokens[:usable]


def create_mdlm_mask_mlx(
    tokens: mx.array, mask_token_id: int, eps: float, progress: float = 0.0, curriculum: bool = False,
) -> tuple[mx.array, mx.array, mx.array, mx.array]:
    """Create masked input for MDLM training (MLX version).
    Returns (masked_ids, original_ids, mask_float, mask_ratio).

    If curriculum=True, sample t from Beta(alpha, beta) that shifts from easy
    (mostly low t) to hard (mostly high t) as progress goes from 0 to 1.
    """
    B, S = tokens.shape
    if curriculum:
        # Beta distribution: starts peaked near 0 (easy), ends peaked near 1 (hard)
        # alpha = 1 + 0.5*progress, beta = 4 - 3*progress
        alpha = 1.0 + 0.5 * progress
        beta_param = 4.0 - 3.0 * progress
        # MLX doesn't have Beta distribution, so use the inverse CDF trick:
        # sample u ~ Uniform, then use numpy to compute Beta inverse CDF
        u_np = np.random.uniform(size=(B,)).astype(np.float32)
        from scipy.stats import beta as beta_dist
        t_np = beta_dist.ppf(u_np, alpha, beta_param).astype(np.float32)
        t_np = np.clip(t_np, eps, 1.0)
        t = mx.array(t_np)
    else:
        # Uniform(eps, 1)
        t = mx.random.uniform(shape=(B,)) * (1.0 - eps) + eps  # [B]
    # Per-position Bernoulli mask
    rand_vals = mx.random.uniform(shape=(B, S))
    mask = (rand_vals < t[:, None]).astype(mx.float32)  # [B, S]
    # Build masked input using mx.where
    masked_ids = mx.where(mask.astype(mx.bool_), mx.array(mask_token_id, dtype=tokens.dtype), tokens)
...
def loss_and_grad_chunked(
    ...
    for chunk_tokens in chunk_sizes:
        tokens = train_loader.next_batch(chunk_tokens, args.train_seq_len)
        masked_ids, original_ids, mask, t = create_mdlm_mask_mlx(
            tokens, args.mask_token_id, args.mask_ratio_eps,
            progress=progress, curriculum=args.curriculum,
        )
        loss, grads = compiled_loss_and_grad(masked_ids, original_ids, mask, t)
        scale = float(tokens.size) / total_tokens
...
def eval_val(
    ...
    K = 8
    mask_ratios = np.array([0.05, 0.1, 0.2, 0.35, 0.5, 0.65, 0.8, 0.95], dtype=np.float64)
    ...
    ce_sums = np.zeros(K, dtype=np.float64)
    masked_counts = np.zeros(K, dtype=np.float64)
    total_bytes = 0.0
    ...
    for k_idx in range(K):
        ...
        mx.random.seed(rng_seed + k_idx * 1000)  # deterministic per ratio
        for batch_seq_start in range(0, total_seqs, val_batch_seqs):
            ...
            original_ids = mx.array(chunk.reshape(-1, args.train_seq_len), dtype=mx.int32)
            B, S = original_ids.shape

            # Deterministic masking at fixed ratio
            rand_vals = mx.random.uniform(shape=(B, S))
            mask_float = (rand_vals < t_val).astype(mx.float32)
            masked_ids = mx.where(mask_float.astype(mx.bool_), mx.array(args.mask_token_id, dtype=mx.int32), original_ids)

            t_tensor = mx.full((B,), t_val, dtype=mx.float32)
            # forward returns (1/t)-weighted loss. Multiply by t to recover raw CE.
            weighted_loss = compiled_loss(masked_ids, original_ids, mask_float, t_tensor).astype(mx.float32)
            mx.eval(weighted_loss)
            raw_ce = float(weighted_loss.item()) * t_val

            num_masked = float(mx.sum(mask_float).item())
            ce_sums[k_idx] += raw_ce * num_masked
            masked_counts[k_idx] += num_masked

            # Count bytes once (first quadrature point)
            if k_idx == 0:
                original_np = chunk.reshape(-1, args.train_seq_len)
                tgt_ids = original_np.reshape(-1)
                prev_ids = np.concatenate(
                    [np.zeros((original_np.shape[0], 1), dtype=np.int32),
                     original_np[:, :-1]], axis=1
                ).reshape(-1)
                ...
                total_token_count += float(tgt_ids.size)

        if log_fn is not None:
            log_fn(f"val_progress:ratio {k_idx + 1}/{K} t={t_val:.2f}")

    # Average raw CE at each mask ratio, then trapezoidal integration of (1/t)*CE(t)
    avg_ce = ce_sums / np.maximum(masked_counts, 1)  # [K]
    integrand = avg_ce / mask_ratios  # (1/t) * CE(t)
    val_loss = float(np.trapz(integrand, mask_ratios))  # nats per token

    bits_per_token = val_loss / math.log(2.0)
    val_bpb = bits_per_token * (total_token_count / total_bytes)
...
def main() -> None:
    ...
    compiled_loss = mx.compile(
        lambda mi, oi, m, t: model.loss(mi, oi, m, t), inputs=model.state, outputs=model.state
    )
    compiled_loss_and_grad = mx.compile(
        nn.value_and_grad(model, lambda mi, oi, m, t: model.loss(mi, oi, m, t)),
        inputs=model.state,
        outputs=model.state,
    )
    ...
    log(f"val_loader:shards pattern={args.val_files} tokens:{val_tokens.size}")
    ...
    if args.warmup_steps > 0:
        ...
        warm_val_seqs = min(val_batch_tokens // args.train_seq_len, val_tokens.size // args.train_seq_len)
        warm_chunk = val_tokens[: warm_val_seqs * args.train_seq_len]
        warm_tokens = mx.array(warm_chunk.reshape(-1, args.train_seq_len), dtype=mx.int32)
        warm_masked, warm_orig, warm_mask, warm_t = create_mdlm_mask_mlx(
            warm_tokens, args.mask_token_id, args.mask_ratio_eps,
        )
        warm_val_loss = compiled_loss(warm_masked, warm_orig, warm_mask, warm_t)
    ...
    while True:
        ...
        for _ in range(args.grad_accum_steps):
            ...
            loss, grads = loss_and_grad_chunked(args, train_loader, compiled_loss_and_grad, progress=progress)
            accum = accumulate_flat_grads(accum, grads, grad_scale)
